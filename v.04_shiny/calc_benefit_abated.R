...
calc_benefit_abated <- function(work_income,partner_earnings) {
  ...
  ### Abate payments. ### Edit 24/09/2024, updated with new information 
  
  if(RA_Abate == 1){
  
    ### abate these benefits 
  residual <- abate -  (JSP + PP_Pay - max(JSP + PP_Pay - abate, 0))
  JSP <-   max(JSP - abate, 0) 
  PP_Pay <-  max(PP_Pay - abate, 0)
  
  ### abate RA and ES (note that this will not do anything if they are 0, because they 
  ### are occuring through FTB A)
  residual_2 <- residual - (RA - max(RA - residual, 0))
  RA <- max(RA - residual, 0)
  ES <- max(ES - residual_2, 0)
  residual_3 <- residual_2 - (ES - max(ES - residual_2, 0))
  
  PA <- ifelse(residual_3 < PA, PA, 0) 
  ...
  return(list(taxable_benefit = max(taxable_benefit,0), net_benefit = max(net_benefit,0), 
              ...
              supp_benefit = supp_benefit, PA = max(PA, 0) ))
}
