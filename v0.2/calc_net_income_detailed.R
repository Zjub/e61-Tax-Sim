...
calc_net_income_detailed <- function(wage_rate, hours, HECSon, Medicare_levy_on) {
  ...
  medicare_levy <- ifelse(Medicare_levy_on == 1, calc_income_tax(gross_income,work_income)[["medicarelevy"]], 0) 
  ...
  net_income <- gross_income - income_tax - medicare_levy - HECS_payment + net_fam_a + net_fam_b + ((supp_benefit) * 26)
