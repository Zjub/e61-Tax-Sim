...
plot_income_highly_detailed <- ggplot() +
  geom_area(data = all_incomes_long, aes(x = hours, y = amount / 1000, fill = income_type), position = "stack") +
  geom_line(data = incomes_data_hourly_highly_detailed, aes(x = hours, y = net_income / 1000, colour = "Net Income"), size = 2) +
...
plot_EMTR_Highly_Detailed <- ggplot() +
  geom_area(data = all_ratios_long, aes(x = hours, y = amount, fill = income_type), position = "stack") +
  scale_colour_manual(values = c("Net Income" = "black")) +
  labs_e61(title = "EMTR Decomposition by Hours Worked",
           x = "Hours Worked",
           y = "EMTR",
           fill = "Income Type",
           colour = "") + add_baseline()  + theme(legend.position = "bottom") + scale_fill_e61() + 
