...
SIH2019_Person$partner_earnings <- SIH2019_Person$HHPrivateEarnings - SIH2019_Person$PerPrivateEarnings + 
                                  (SIH2019_Person$ITGCBCU8 -SIH2019_Person$ITAGCBC8 )
...
write.csv( final_results, "SIHComparisonCRA_updated.csv")




############################################## Create an income distribution for the charts. 
variables_to_keep <- c("over_60", "partnered", "living_alone", "Home_owner", 
                       "young_child", "Numb_dep", "Have_dep", "Main_carer_dep", 
                       "PPeligible", "IWSSUCP8")

# Create a new dataframe with only the specified variables
SIH2019_Person_filtered <- SIH2019_Person[ , variables_to_keep]

# View the new dataframe to ensure it has the correct variables
head(SIH2019_Person_filtered)

write.csv(SIH2019_Person_filtered, "SIH_backing_data.csv")
