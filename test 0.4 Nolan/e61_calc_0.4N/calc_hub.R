...
calc_hub <- function(wage_rate, hours, HECSon, Medicare_levy_on) {
  ...
  RA <- ifelse(net_fam_a == 0, calc_benefit_abated(work_income,partner_earnings)[["RA"]] * 26, 
            calc_family_benefit(child_age,gross_fam_income,work_income,taxable_benefit)[["RA"]] * 26)
  ES <- max(calc_benefit_abated(work_income,partner_earnings)[["ES"]], 
            calc_family_benefit(child_age,gross_fam_income,work_income,taxable_benefit)[["ES"]]) * 26
  
  PA <- calc_benefit_abated(work_income,partner_earnings)[["PA"]] * 26
  
  supp_benefit <- RA + ES + PA 
  
  net_income <- gross_income - income_tax - medicare_levy - HECS_payment +
                 net_fam_a + net_fam_b + supp_benefit
