...
calc_family_benefit <- function(child_age,gross_fam_income,work_income,taxable_benefit){
  ...
  if((policy_quarter < 3 & policy_year <= 2016) | policy_year <= 2015){
  ...
  fam_a_ann_supp <- ifelse(ATI < fam_ftba_eoy_incthreshold, 
  ...
  if (taxable_benefit > 0) {
    ...
    First_abatement <- max(min(ATI - fam_max_threshold, 
                               fam_base_threshold - fam_max_threshold), 0) 
